...
export default function Lock({ state }) {
  ...
  return (
    <>
      ...
      <span
        className={`notification rounded-full color-white absolute -top-2 -left-4 bg-secondary-darker text-white w-6 h-6 flex justify-center items-center ${
          newToModeration > 0 ? "" : null
        }`}
      >
        {newToModeration > 0 ? newToModeration : null}
      </span>
    </>
  );
}
